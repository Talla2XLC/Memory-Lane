  ...
  render() {
    ...
    const personItems = userPersons.map((item) => (
      <div key={item.id}>
        <Link className="persons__link" to={`/persons/${item.id}`}>
          <img
            className={this.state.styleType + "__img"}
            src={
              item.ico_url.length > 0
                ? item.ico_url
    ...
    return loading ? (
      ...
      <div className="persons">
        <Sorting currentPage="persons" setGridType={this.setGridType} />
        <div className={this.state.styleType}>{personItems}</div>
      </div>
    );
  }
}
