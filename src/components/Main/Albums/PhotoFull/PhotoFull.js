...
class PhotoFull extends Component {
  ...
  render() {
    ...
    const tags = this.props.location.props.tags
      ? JSON.parse(this.props.location.props.tags)
      : false;
    const persons = this.props.location.props.persons
      ? JSON.parse(this.props.location.props.persons)
      : false;
    const coords = coordinates ? JSON.parse(coordinates) : false;

    const img = <img className={"img"} src={url} alt="gallery_pic" />;

    const faceBorder = coords
      ? coords.map((params, index) => (
          <div
            key={index}
            className="face"
            style={{
            ...
            {persons ? (
              <>
                <span className="face-span">
                  {
                    personsList.find((pers) => pers.id === persons[index])
                      .first_name
                  }
                </span>
                <span className="face-span">
