          ...
          <div className="imgWrap">
            {album.photo ? (
              <img
                ...
                src={album.photo[album.photo.length - 1].content_url}
                alt="albumPreview"
              />
            ) : (
              <div className="album-empty">
                <FamilySvg className="album-empty-svg" />
                <span className="album-empty-txt text3">
                  Здесь пока нет ни одной фотографии
                </span>
              </div>
            )}
          </div>
    ...
    return loading ? (
      ...
      <div className="albums-container">
        <Sorting
          ...
          performAction={this.performAction}
        />
        <div className={this.state.styleType}>{albumsItems}</div>
      </div>
    );
  }
}
